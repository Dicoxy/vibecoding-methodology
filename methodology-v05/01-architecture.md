...
Каждый агент также:
  - Пишет changelog (формат и правила: 03-changelog-rag.md, раздел 3.2)
...
Приёмка отделена от исполнения и от оркестрации — иначе оценивающий заинтересован в результате оценки.

- **Судья архитектуры** проверяет контракт ДО передачи исполнителю: исполнимость, непротиворечивость, проверяемость, соответствие цели.
- **Судья результата** выносит бинарный вердикт по гарантиям контракта ПОСЛЕ исполнения.

Обе позиции разлагаются на автоматизируемую механику и остающуюся в диаде ответственность. Судья — сам артефакт и подлежит приёмке (мутационное тестирование) до того, как его вердикту начнут верить.

Подробно: 04-validation.md, разделы 4.11–4.13.
...
Подробно: 03-changelog-rag.md, разделы 3.4–3.9
```
...
Агент создаёт:
  ...
  3. Changelog-запись (формат: 03-changelog-rag.md, раздел 3.2)
...
### Читаемость стыка (кандидат в принцип, 11.09.2026)

**Латентная связь — деталь реализации блока, а не тип стыка.**

Повод: связки моделей, передающих друг другу скрытые состояния напрямую, минуя текст (Mostik, сентябрь 2026: замороженные GLM + Qwen через обучаемый «мост»). Для методологии такая связка — один блок с одним контрактом: вход — запрос, выход — ответ, метрики — качество, цена, латентность. Что внутри — мост, дистилляция, одна модель — архитектору безразлично. Это прямое следствие манифеста: движок внутри блока заменяем.

Граница: на стыке между блоками канал обязан быть читаемым — текст, JSON, схема. Причины:

- **Проверяемость.** Принцип 5 манифеста: ошибки живут на стыках, стык проверяется одним контрактом. На вектор скрытого состояния контрактный тест не написать — «на входе X» не формулируется.
- **Заменяемость.** Скрытое состояние привязано к паре конкретных моделей и версии моста. Сменил модель — мост переобучается, контракт умирает. Читаемый стык переживает замену любого из блоков.
- **Аудит.** Читаемый стык — это и есть контракт: его читает человек, он попадает в CHANGELOG и RAG. Слой ФАКТЫ проверяется по нему.

«Узкое горлышко» текстового обмена между блоками — не дефект, а свойство: сжатие до контракта делается намеренно.

Статус: кандидат. Принятие — когда появится forcing function (см. TASKS #34), не когда написан раздел.
...
→ **Перенесён в [03-changelog-rag.md](03-changelog-rag.md), раздел 3.2**

Краткая суть: changelog-запись создаётся в два этапа (ПЛАН до работы, ФАКТ после). Принцип Манифеста: документирование до, а не после. Формат строгий — RAG индексирует по структуре.

---

*Версия 1.2 | 19 февраля 2026*
*Обновлено: секция 1.4 перенесена в 03-changelog-rag.md, ссылки на changelog обновлены*
