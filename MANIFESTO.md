...
Перед декомпозицией на блоки и контракты система должна иметь чётко определённую цель. Цель — это ответ на вопрос «какой результат мы должны получить и как мы узнаем, что он достигнут».

Архитектор формулирует цель на языке бизнеса. Оркестратор превращает её в измеримые критерии. Каждый блок наследует свою часть цели — не только «что делает», но и «зачем существует».

Четыре слоя проверки, каждый проверяется предыдущим:

```
ЦЕЛЬ         — куда идём (правильную ли систему мы строим?)
  ↓
СТРУКТУРА    — что делаем (все ли части на месте?)
  ↓
КОНТРАКТЫ    — как стыкуются (совместимы ли блоки?)
  ↓
ФАКТЫ        — что реально сделано (соответствует ли реализация плану?)
```

Цель защищает от двух опасностей: сползания в ненужные фичи (если фича не ведёт к цели — она не нужна) и потери направления (если непонятно что делать дальше — сверься с целью).
...
- **Зачем существует** — цель блока, унаследованная от цели системы
...
│   Философия: почему блоки, контракты, цель, runtime-agnostic
...
│   │
│   ├── ШАБЛОНЫ (templates/)
│   │   CONTRACT.md, SPEC.md, GOAL.md, CLAUDE.md
│   │
│   ├── ПАТТЕРНЫ (patterns/)
...
*Два принципа, проверенные практикой. При следующей ревизии манифеста подлежат интеграции в основную структуру.*

### Сверка с миром

Любая модель ИИ отстаёт от мира — и при этом уверенно строит решения на устаревшем знании. Память человека — ненадёжный триггер для проверки. Поэтому каждое структурное решение проходит разведку актуального состояния индустрии *до* фиксации, а в запись решения входит «состояние мира на дату»: что существует, что выбрали, почему своё или чужое.

Полагаться на память архитектора — тот же антипаттерн, что документировать после, а не до. Система, которая не сверяется с миром, вечно строит вчерашние решения.

### Ответственность не делегируется

Каждая роль системы распадается на два слоя. **Механика** — диспетчеризация, измерение, исполнение — автоматизируется полностью. **Мышление и ответственность** — остаются у человека, в диаде архитектор + ИИ.

Агенты работают, измеряют и эскалируют. Принимает — человек. Судья-машина может вынести вердикт «контракт выполнен», но решение «запускаем» и ответственность за него не циркулируют между агентами — они сосредоточены в одной точке.

Пока человек несёт ответственность за результат, созданный ИИ, — он получает результат. Как только он перекладывает ответственность на ИИ — он перестаёт получать что бы то ни было.

---

## Резюме в одном абзаце

Блочный кодинг — это методология, в которой системы строятся из блоков с чёткими контрактами, а не из строк кода. Человек-архитектор определяет *что* нужно и *зачем*, ИИ-реализатор определяет *как* это сделать. Цель системы — якорь, от которого наследуется цель каждого блока. Спецификации не привязаны к языку, тесты проверяют поведение, контроль качества основан на метриках. Эта архитектура готова к любому будущему: будь то генерация кода, генерация бинарников или что-то, чего мы пока не можем представить. Потому что правильная декомпозиция и чистые контракты — это не про технологию. Это про мышление.
